...
def test(request):
    """Ренедер страницы с тестом. Версия Булата"""
    ...
    for question in questions:
        ...
        q += 1
    return render(request, 'exam_testing/test.html', context={'dict_of_question': dict_of_questions,
                                                              'dict_of_questions_answers': dict_of_questions_answers})
...
def test2(request):
    ...
    return render(request, "exam_testing/test.html", {"questions": questions})
...
def headpage(request):
    ...
    for test in Test.objects.order_by("-id")[0:15]:
        ...
        for tst_qst in TestQuestions.objects.filter(test=test):
            first = list(QuestionAnswer.objects.filter(question=tst_qst.question, correct=True))
        ...
        for ot_test in Test.objects.filter(user=test.user).order_by('id'):
            for test_quest in TestQuestions.objects.filter(test=ot_test):
                ...
                right = list(
                    map(lambda x: x.question, UserAttempt.objects.filter(test=ot_test, question=test_quest.question)))
                if left == right:
    ...
    if not user.is_anonymous:
        ...
        for test_2 in user_tests:
            ...
            for tst_qst in TestQuestions.objects.filter(test=test_2):
                ...
                second = list(
                    map(lambda x: x.question, UserAttempt.objects.filter(test=test_2, question=tst_qst.question)))
                if first == second:  # а это работает?
                    done2 += 1
            takes.append(done2)
    ...
    return render(request, 'exam_testing/headpage.html', context={'list_elems': list_attempts,
